...
const restoreUploadedJsonData = (chartData: any) => {
  ...
  const controlGroup = chartData.filter(
    (datapoint: any) => datapoint.group === groupNames[0],
  );
  const uniqueKeys = controlGroup.map((datapoint: any) => datapoint.key);

  for (let i = 1; i < groupNames.length; i++) {
    const groupName = groupNames[i];
    const group = chartData.filter(
      (datapoint: any) => datapoint.group === groupName,
    );
    ...
    for (let j = 0; j < group.length; j++) {
      const { key } = group[j];
      const groupDuplicateKeys = group.filter(
        (datapoint: any) => datapoint.key === key,
      );
      const headerDuplicateKeys = uniqueKeys.filter(
...
export const processDataFile = (file: any) =>
  new Promise<any>((resolve, reject) => {
    ...
    reader.onloadend = (event: any) => {
      ...
      if (file.type === "text/csv") {
        ...
        const maxRowLength: any = (result.data as any).reduce(
          (maxLength: number, row: any) => {
        ...
        if (isDataInvalid) {
          result.data = result.data.map((row: any, rowIndex: number) => {
            ...
            if (rowIndex === 0) {
              return row.concat(
                new Array(maxRowLength - row.length)
                  .fill(0)
                  .map(
                    (_, columnIndex: number) =>
                      `label ${row.length + columnIndex}`,
                  ),
              );
            }
            // Fill dataset rows with null values to max row length.
            return row.concat(
              new Array(maxRowLength - row.length).fill("null"),
            );
          });
        }

        // Check if the csv contains the group header indicating that dataset names are provided.
        const isGrouped = header.some(
          (header: string) => header.toLowerCase() === "group",
        );
...
export const warningNotificationProps = {
  ...
  subtitle:
    "data did not match expected format so we modified it so you can still use it",
};
