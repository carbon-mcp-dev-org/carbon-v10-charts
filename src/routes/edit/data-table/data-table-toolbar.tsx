...
export const DataTableToolbar = ({
  ...
  const generateCsvFile = () => {
    ...
    state.body.forEach((row: any) => {
      const datasetString = row
        .reduce((generatedString: string, datapoint: any, index: number) => {
          if (index === 0) {
          ...
          if (datapoint.value !== undefined) {
            if (datapoint.date !== undefined && isTimeSeries) {
              return generatedString.concat(
                `${datapoint.value}|${datapoint.date},`,
              );
            }
  ...
  return (
    <TableToolbar>
      ...
      >
        <TableToolbarAction
          onClick={() => downloadFile(generateCsvFile(), "text/csv")}
        >
          Export as .csv
        </TableToolbarAction>
        <TableToolbarAction
          onClick={() => downloadFile(generateJsonFile(), "application/json")}
        >
          Export as .json
        </TableToolbarAction>
      </TableToolbarMenu>
