...
let updatePreviewUrl = async () =>
  console.log("updatePreviewUrl not initialized yet");
  ...
  return (
    <Modal
      ...
      open={
        ...
        props.displayedModal === ShareOptionsModals.IMAGE_EXPORTS
      }
    ...
    >
      <p>
        Export a static image of this chart for use in presentation decks or
        designs.
      </p>
      <div
        style={{
...
const ExportModalSettings = ({ inputs, handleChange }: any) => {
  ...
  const numInputOnClick = (event: any) => {
    if (event.imaginaryTarget && event.imaginaryTarget.value) {
      onDimensionChange(
        event.imaginaryTarget.id,
        parseFloat(event.imaginaryTarget.value),
      );
    }
  };
