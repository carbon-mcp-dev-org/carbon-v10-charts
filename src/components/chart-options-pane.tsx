...
export const ChartOptionsPane = ({ chart, setChart }: any) => {
  ...
  const setOverrideVariantsCount = (override: boolean) => {
    if (!override) {
      ...
      delete updatedChart.options?.rawChartOptions?.color?.pairing
        ?.numberOfVariants;
  ...
  useEffect(() => {
    const numberOfVariants = Number(
      getFieldValue("color-pairing-numberOfVariants", 0),
    );

    if (
      numberOfVariants &&
      getGroupNames(chart.data).length !== numberOfVariants
    ) {
  ...
  const setStacked = (axes: any, stacked: boolean) => {
    if (
      axes?.left &&
      (axes?.left.scaleType === "linear" || axes?.left.scaleType === "log")
    ) {
      axes.left = {
        ...axes.left,
        stacked,
    ...
    if (
      axes?.right &&
      (axes?.right.scaleType === "linear" || axes?.right.scaleType === "log")
    ) {
      axes.right = {
        ...axes.right,
        stacked,
    ...
    if (
      axes?.top &&
      (axes?.top.scaleType === "linear" || axes?.top.scaleType === "log")
    ) {
      axes.top = {
        ...axes.top,
        stacked,
    ...
    if (
      axes?.bottom &&
      (axes?.bottom.scaleType === "linear" || axes?.bottom.scaleType === "log")
    ) {
      axes.bottom = {
        ...axes.bottom,
        stacked,
  ...
  const handleChange = (inputID: string, inputValue: any) => {
    ...
    if (inputID === "type") {
      ...
      // the container.
      setStacked(
        updatedChart.options.rawChartOptions.axes,
        inputValue.includes("stacked"),
      );
    }
    ...
    >
      <AccordionItem title="General">
        ...
        >
          ...
          <Checkbox
            ...
            onChange={(event: any) => handleChange("tooltip-enabled", event)}
          />
          ...
          {chart.type !== "donut-chart" && chart.type !== "pie-chart" && (
      ...
      <AccordionItem title="Colors">
        <FormGroup
          ...
          legendText=""
        >
          <Tooltip triggerText="Number of variants" direction="top">
            <p>Number of distinct colors in the palette.</p>
          </Tooltip>
          <div
            className={css`
              display: flex;
          ...
          >
            ...
            <Checkbox
              ...
              onChange={(event: any) => setOverrideVariantsCount(event)}
            />
          </div>
          <Select
            className={inputWrap}
          ...
          >
            {getPaletteOptions().map((value) => (
              <SelectItem text={value.toString()} value={value} key={value} />
            ))}
          </Select>
        </FormGroup>
      </AccordionItem>
      <AccordionItem title="X axis">
        ...
        >
          <ContentSwitcher
            className={inputWrap}
            onChange={(event: any) =>
              setState({ XAxisContentSwitch: event.name })
            }
            selectedIndex={0}
            size="md"
          >
      ...
      <AccordionItem title="Y axis">
        ...
        >
          <ContentSwitcher
            className={inputWrap}
            onChange={(event: any) =>
              setState({ YAxisContentSwitch: event.name })
            }
            selectedIndex={0}
            size="md"
          >
            <Switch name="left" text="Left"></Switch>
            <Switch name="right" text="Right"></Switch>
          </ContentSwitcher>
          {SwitchAxisType(
            state.YAxisContentSwitch ? state.YAxisContentSwitch : "left",
          )}
        </FormGroup>
      </AccordionItem>
